...
public interface IVisualNode : INode<GraphRuntime<IVisualNode>>
{
    float GetValue([NotNull] GraphRuntime<IVisualNode> graph);
    float GetValue([NotNull] GraphRuntime<IVisualNode> graph, [NotNull] string port);
}

public abstract class VisualNode : IVisualNode
{
    public abstract float GetValue(GraphRuntime<IVisualNode> graph);
    public abstract float GetValue(GraphRuntime<IVisualNode> graph, string port);
    ...
    protected IEnumerable<float> GetConnectedValues(GraphRuntime<IVisualNode> graph, string port)
    {
...
public class ValueNode : VisualNode
{
    ...
    public override float GetValue(GraphRuntime<IVisualNode> graph, string port)
    {
        Assert.IsTrue(port == nameof(_outPort));
        return GetValue(graph);
    }

    public override float GetValue(GraphRuntime<IVisualNode> graph)
    {
        return Value;
    }
}
